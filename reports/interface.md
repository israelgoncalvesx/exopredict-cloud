...
- **Classe real escondida até depois da previsão**: mostrar a resposta antes de clicar em "Classificar" estragava a demonstração. Agora só aparece depois do resultado, como comparação (✅/❌) com a previsão do modelo.
- **Explicação em linguagem simples após o resultado** (`gerar_explicacao`): não é SHAP ao vivo (pesaria demais na API enxuta, `reports/api.md`) — é uma leitura heurística fundamentada nos achados já documentados (`reports/explainability.md`, `reports/eda.md` bloco 7). Cuidado deliberado: para a fronteira `CONFIRMED`/`CANDIDATE` (a mais difícil do modelo, ~85% dos erros — `reports/evaluation.md`), o texto descreve os valores observados sem soar mais confiante do que o modelo realmente é. A primeira versão dizia algo como "sinal forte, associado a maior confiança de confirmação" mesmo quando o modelo previa `CANDIDATE` — parecia contradizer a própria previsão. Corrigido para reconhecer a ambiguidade da fronteira em vez de fingir uma certeza que o modelo não tem.
